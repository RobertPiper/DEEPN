...
# v3 of run_Y2H_enrichement_stats.R: same collated-input format and the same
# group-building/DESeq2-fitting logic (both already generic over however
# many CONFIG_START rows exist), plus one new piece - a specificity
# contrast (Bait1-selected vs Bait2-selected, ported in spirit from
# Y2H-SCORES' calc_spec_score() in Software/spec_score.R, but scoped down
# to exactly one pairwise comparison since this build only ever has two
# baits, not spec_score.R's general N-bait combn() case) - and an output
# writer that adds Bait2's and the specificity contrast's columns when
# Bait2 is present, while staying identical to the v2 output when it isn't.
#
