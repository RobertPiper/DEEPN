...
# v4 of run_Y2H_enrichement_stats.R: adds Bait-vs-Vector as a direct DESeq2
# contrast (pvalue_bait1_vs_vector/log2FoldChange_bait1_vs_vector, and
# bait2_vs_vector when Bait2 is present) - the one comparison v3 never
# computed at all.
#
# v3's specificity contrast (Bait1-selected vs Bait2-selected) was ported
# from Y2H-SCORES' calc_spec_score() (Software/spec_score.R), which is
# actually general over N sample groups - it runs combn(groups, 2) and
# produces a DESeq2 contrast for every pair. v3's port scoped that down to
# exactly the Bait1-vs-Bait2 pair ("this build only ever has two baits"),
# which quietly excluded Vector from ever being one of the compared groups
# - even though Vector was available the whole time and calc_spec_score()
# would have contrasted it against each bait automatically if included.
# v4 restores that: Vector is now one of the groups being compared, so
# Bait1-vs-Vector and Bait2-vs-Vector exist as real pairwise contrasts,
# computed the same way Bait1-vs-Bait2 always was (a single results() call
# against the one already-fit DESeq2 model - no re-fit, no new dispersion
# estimation, just one more contrast pulled from the same fit).
#
# Positive log2FoldChange_baitN_vs_vector = more abundant under that bait's
# selection than under Vector's selection (enriched relative to
# background) - unlike pvalue_bait1/log2FoldChange_bait1, which contrasts a
# bait's Selected sample against that SAME bait's own Non-Selected sample
# and is dominated by Y2H selection's library-complexity collapse (nearly
# every gene, real hits included, comes out strongly negative there -
# confirmed on real data: a gene with Bayesian pBait1_Vec > 0.9999 showed
# log2FoldChange_bait1 = -2.0). That column was never a usable proxy for
# "enriched vs vector" and should not be read as one.
#
...
# ---------- bait-vs-vector contrast(s) (new in v4) ----------
