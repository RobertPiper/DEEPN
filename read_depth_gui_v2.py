...
ui_path = os.path.join('ui', 'Read_Depth_v2.ui')
if sys.platform == 'win32':
    ui_path = os.path.join('ui', 'Windows', 'Read_Depth_v2.ui')
...
class Read_Depth_Gui(QtWidgets.QMainWindow, form_class):
    ...
    def __init__(self, folder, directory, gene_list_file, *args):
        ...
        self.read_list_thread.GeneListFinished.connect(self.dataset_loaded)
        self.calculate_depth_thread = CalculateDepth_Thread(self)
        self.calculate_depth_thread.CalculateDepthFinished.connect(self.load_table_w_data)
    ...
    def populate_accession_suggestions(self):
        ...
        self.accession_number_search.editingFinished.connect(self.search_changed)
    ...
    def search_changed(self):
        ...
        search_text = str(self.accession_number_search.text()).strip()
        if search_text in self.gene_info_list.keys():
            ...
            self.accession_number_search.setText(self.selected_gene_name)
            for accession in self.gene_info_list[self.selected_gene_name]:
                self.accession_list.addItem(accession)
    ...
    def accession_changed(self):
        self.selected_accession_number_name = str(self.accession_list.currentText())
        if self.selected_accession_number_name == '':
            return
        try:
            self.selected_gene = self.gene_list[self.selected_accession_number_name]
            self.read_list_thread.selected_gene = self.selected_gene
            _sequence = self.sequence_format_html(self.selected_gene)
            self.sequence_browser.setHtml(_sequence)
    ...
    def load_table_w_data(self):
        ...
        for i, data in enumerate(self.results):
            ...
            self.results_lst.setItem(i, 2, QtWidgets.QTableWidgetItem(data[2]))
